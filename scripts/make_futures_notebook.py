...
def build(rolls: dict[str, float]) -> dict:
    cells = [
        md("# 台指期（TX）快速修復回檔入場策略 — FinLab 回測（Colab 版）",
           ...
           "",
           "預設注碼是 **最終套件**（docs/final_package.md）：進場日收盤 < MA200 固定 3x、",
           "≥ MA200 半預算（4% ÷ 停損距離，≤2.5x）、權益 +150% 減碼到 1x、",
           "空手時核心 0.5x（遲滯帶 ±2%）。第 6 節可切回舊版注碼。",
           "",
        ...
        md("## 6. 選擇參數",
           ...
           "| `MAX_LEVERAGE` | 5.0 | 風險式槓桿上限（均線上規則用一半：2.5x） |",
           "| `SAME_DAY` | True | 當天期貨收盤成交；False 改隔一個交易日收盤（對照用） |",
           "| `SIZING` | `hybrid` | 最終套件的混合注碼；`risk` 為舊版 8% ÷ 停損距離 |",
           "| `STEP_DOWN` | True | 權益 +150% → 減碼到 1x |",
           "| `CORE` / `CORE_BAND` | 0.5 / 0.02 | 空手期核心槓桿與遲滯帶；`CORE = 0` 不加 |",
           "",
           "對應 repo 指令 `python3 scripts/futures_trades.py`（預設值完全相同）。"),
        code("from tw_backdraw.config import PRESETS",
             "",
             "PRESET = 'tuned'       # ← 想換參數組改這裡",
             "MAX_LEVERAGE = 5.0     # 風險式槓桿上限（均線上規則用它的一半：2.5x）",
             "SAME_DAY = True",
             "SIZING = 'hybrid'      # 'hybrid'＝最終套件（均線下固定 3x／均線上 4%÷停損距離）；'risk'＝舊版 8%÷停損距離",
             "STEP_DOWN = True       # 權益 +150% → 減碼到 1x（§22）",
             "CORE = 0.5             # 空手期核心槓桿；0 = 不加核心",
             "CORE_BAND = 0.02       # 核心遲滯帶 ±2%（§23）",
             "",
             "cfg = PRESETS[PRESET]",
             "print(f'訊號   回檔 ≥{cfg.setup.min_drawdown:.0%}，谷底起 ≤{cfg.setup.max_repair_bars} 日'",
             "      f'內補回 ≥{cfg.setup.repair_fraction:.0%}')",
             "if SIZING == 'hybrid':",
             "    print(f'注碼   進場日收盤 < MA200 → 固定 3x；≥ MA200 → '",
             "          f'{cfg.sizing.risk_per_trade / 2:.0%} ÷ 停損距離，上限 {MAX_LEVERAGE / 2:g}x')",
             "else:",
             "    print(f'注碼   舊版：{cfg.sizing.risk_per_trade:.0%} ÷ 停損距離，上限 {MAX_LEVERAGE:g}x')",
             "print('減碼   ' + ('權益 +150% → 降到 1x' if STEP_DOWN else '關閉'))",
             "print('核心   ' + (f'空手時 {CORE:g}x，站上 MA200×{1 + CORE_BAND:.2f} 建、跌破 ×{1 - CORE_BAND:.2f} 平'",
             "               if CORE else '不加'))",
        ...
        md("## 7. 跑策略",
           ...
           "槓桿依注碼規則決定、進出場各扣一次成本（期交稅十萬分之二＋每口 50 元），",
           "持有期間權益線性於期貨報酬（固定口數，不複利）；step-down 觸發時平掉多出的口數。",
           "",
           "`nav_pos` 是策略部位本身的淨值（與 repo 的 `scripts/futures_trades.py` 完全一致）；",
           "`nav` 再疊上空手期核心，是完整套件的淨值，供報表與績效對照使用。"),
        code("from tw_backdraw.engine import Engine, moving_average",
             "from tw_backdraw.futures import (FuturesCost, core_overlay, entries_from_trades,",
             "                                 hybrid_entries, trade_details, vehicle_series)",
             "",
             ...
             "                             moving_average(bars, 200), step_gain=step,",
             "                             step_leverage=1.0, band=CORE_BAND)",
             "else:",
             "    nav = nav_pos",
             "details = trade_details(dates, nav_pos, entries, detail)",
             "",
             "print(f\"{'進場':<12}{'出場':<12}{'停損距離':>9}{'槓桿':>7}{'減碼日':>12}\"",
             "      f\"{'期貨報酬':>10}{'權益報酬':>10}\")",
             "print('-' * 74)",
             "for t in detail:",
             ...
             "      f'範圍 {min(levs):.2f}~{max(levs):.2f}　'",
             "      f'減碼 {sum(1 for t in detail if t.step_date)} 筆')"),
        ...
        code("import numpy as np",
             ...
             "             name=f'台指期快速修復 {PRESET}（{SIZING}{\"＋核心\" if CORE else \"\"}）',",
             "             upload=False)",
        ...
        code("def series_metrics(s):",
             ...
             "rows = {f'完整套件（{SIZING}{\"＋核心\" if CORE else \"\"}）': series_metrics(eq),",
             "        '策略部位本身（不含核心）': series_metrics(pd.Series(nav_pos, index=di)),",
        ...
        md("## 12.（選配）與舊版注碼、濾網版對照",
           "",
           "舊版＝8% ÷ 停損距離、上限 5x、不減碼、不加核心（2026-09 前每日訊息的注碼）；",
           "濾網版＝只做進場日收盤低於 MA200 的訊號＋固定 3x（docs/strategy.md §18）。",
           "兩者都是同一批訊號，只有注碼不同。證據與取捨見 §20–§23。"),
        code("from tw_backdraw.futures import fixed_leverage, trend_filter",
             "",
             "ent_r = entries_from_trades(result.trades, bars, cfg, MAX_LEVERAGE, SAME_DAY)",
             "nav_r, _ = vehicle_series(dates, fseries, ent_r, FuturesCost(), closes)",
             "ent_d = fixed_leverage(trend_filter(ent_r, bars, 200, below=True), 3.0)",
             "nav_d, _ = vehicle_series(dates, fseries, ent_d, FuturesCost(), closes)",
             "print(f'濾網版：只做收盤低於 MA200 的訊號　{len(ent_r)} → {len(ent_d)} 筆')",
             "rows_d = {'完整套件（本 notebook 主結果）': series_metrics(eq),",
             "          '舊版 8%÷停損距離≤5x': series_metrics(pd.Series(nav_r, index=di)),",
             "          '濾網 MA200 ＋固定 3x': series_metrics(pd.Series(nav_d, index=di))}",
